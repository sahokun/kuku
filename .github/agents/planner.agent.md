...
Explore the codebase directly to gather context, find analogous existing features to use as implementation templates, and surface potential blockers or ambiguities. Apply the search strategy and speed principles below. If Usable, launch **2-3 *Explore* subagents in parallel** — one per area — to speed up discovery. Update the plan with your findings.
